...
横幅继续使用轻量 SVG，并恢复早期的紫色 Midnight Sync 拓扑方向。中文与英文
分别使用 `codexkeep-banner.svg` 和 `codexkeep-banner.en.svg`；两张图共享构图、
图形、尺寸和色彩，仅替换承担语义的界面标签与替代文本。

- 英文横幅完整恢复提交 `4d9d591` 中 `codexkeep-banner.svg` 的设计，不修改
  画布、面板、节点、标签数量、尺寸、间距、颜色或字体参数；唯一允许的新增
  内容是下述 `sync-trails` 装饰图层；
- 中文横幅以该英文原版为唯一母版，只翻译可见文字与无障碍说明，不重新设计
  构图，也不根据同步范围增删标签；
- 左侧展示 `CodexKeep` 品牌名、两行产品说明，以及原版的三个圆角标签：
  `portable config`、`private by default` 和 `recoverable writes`；
- 右侧保持“THIS MAC → Private Git → NEW MAC”三个节点，私有 Git 节点继续
  使用原版锁形图标和紫色视觉焦点；
- 底部保留 `skills · agents · preferences` 与
  `credentials stay local` 两条边界说明；
- 使用原版低对比网格、紫色与蓝色光晕、卡片阴影和绿色完成状态；
- 横幅即使加载失败，紧邻的 Markdown 文案也必须独立表达完整用途；
- 保持无脚本、可编辑、体积小于 100 KB，并提供准确的 SVG title、desc 和图片
  alt。

中文横幅逐项翻译为“可移植 CODEX 配置”“安全同步你的 Codex 世界”“覆盖你使用
的每台设备”“可移植配置”“默认私有”“可恢复写入”“私有同步拓扑”“预检完成”
“本机”“私有 Git”“你的配置基准”“新 MAC”“技能 · 智能体 · 偏好”和“凭据留在
本机”。`CodexKeep`、`Codex`、`Git` 和 `MAC` 保留原样。英文横幅保留提交
`4d9d591` 原版的全部英文标签。

两张横幅固定使用原版 `1600 × 480` 画布。中文版只替换文本节点与无障碍说明，
不得修改字体参数、移动节点、改变标签框宽度或改变整体视觉比例。

左栏三个圆角标签下方增加一个 `id="sync-trails"` 的纯装饰图层，用于填补左栏
提前结束形成的视觉空白：

- 三条极细的数据轨迹分别沿用三个标签的紫色、蓝色和绿色；
- 轨迹从标签下方出发，向右下方舒缓汇聚，末端朝向右侧拓扑卡片；
- 转折和汇聚处只使用少量微光节点与一个克制的方向信号；
- 轨迹位于左栏下部，控制在约 `x=88–690`、`y=338–430` 的范围内；
- 线条与节点使用约 15%–30% 的低透明度，不新增滤镜、文字、卡片或动画；
- 中文与英文必须使用完全相同的 `sync-trails` 图形和参数；
- 移除整个 `sync-trails` 图层后，英文 SVG 必须逐字节恢复为提交 `4d9d591`
  中的历史原图。
...
- 中文横幅不包含 `PRIVATE SYNC TOPOLOGY`、`PREFLIGHT COMPLETE`、
  `THIS MAC`、`NEW MAC` 或 `credentials stay local` 等英文界面标签；
- 英文横幅移除 `sync-trails` 图层后与提交 `4d9d591` 中的原图逐字节一致；
- 中文横幅只有可见文字和无障碍说明与英文母版不同，`sync-trails` 图层完全
  相同；
- 两张横幅保持相同的 `1600 × 480` 画布、三个圆角标签、三个流程节点、坐标、
  字体参数、尺寸、色彩与视觉结构；
- `sync-trails` 不包含文本、脚本或动画，且不会覆盖左栏现有内容或右侧拓扑；
