...
- 英文横幅完整恢复 Git 中上一版 `codexkeep-banner.svg` 的设计，不修改画布、
  面板、节点、标签数量、尺寸、间距、颜色、字体参数或装饰效果；
- 中文横幅以该英文原版为唯一母版，只翻译可见文字与无障碍说明，不重新设计
  构图，也不根据同步范围增删标签；
- 左侧展示 `CodexKeep` 品牌名，以及原版的四个能力标签：`SKILLS`、
  `INSTRUCTIONS`、`AGENTS` 和 `PREFERENCES`；
...
中文横幅逐项翻译为“CODEX 配置同步”“技能”“指令”“智能体”“偏好”“显式 ·
私有 · 可恢复”“同步路径 · 01”“预检就绪”“本机”“预检”“另一台 MAC”“凭据 ·
会话 · 机器路径”和“仅限本机”。`CodexKeep` 与 `GIT` 保留原样。英文横幅保留
原版的全部英文标签。

两张横幅固定使用原版 `1600 × 420` 画布。中文翻译可以在原有文本节点内微调
字距或字号以避免溢出，但不得移动节点、改变标签框宽度或改变整体视觉比例。
...
- 英文横幅与 Git 中上一版原图的视觉和文字完全一致；
- 中文横幅只有可见文字和无障碍说明与英文母版不同；
- 两张横幅保持相同的 `1600 × 420` 画布、四个能力标签、节点数量、坐标、
  尺寸、色彩与视觉结构；
