...
- **可能改动：** 可能安装缺少的第三方 marketplaces 和 plugins、更新
  `plugins.json`、协调可移植 `config.toml` 白名单、备份并修改真实 Codex
  配置、提交本地文件、rebase 远端更新并 push。
- **确认：** 执行写入前展示完整同步计划。`--yes` 接受这份常规计划。
- **冲突：** 不兼容的 marketplace 来源会在受管理内容改动前停止。同一可移植
  设置被本机和远端同时修改时必须明确选边。未解决的 Git 冲突会停止同步，不会
  强制覆盖任意一方。
- **失败与恢复：** 远端离线不妨碍保存本地提交；push 失败时保留本机改动，
  稍后重新运行 `codexkeep sync` 即可。账号绑定的 plugins 只会提示手工安装
  或登录，不会复制凭据。
